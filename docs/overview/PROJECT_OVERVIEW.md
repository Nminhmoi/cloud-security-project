...
CloudBox là ứng dụng web để lưu trữ, quản lý và chia sẻ tài liệu. Giao diện được dựng ở phía máy chủ; giao diện web và REST API dùng chung phiên đăng nhập của Flask.
...
Mục tiêu là giúp người dùng chia sẻ tài liệu thuận tiện, đồng thời kiểm soát được ai có quyền truy cập. Dự án cũng minh họa cách kết hợp xác thực, phân quyền và giám sát trong ứng dụng với cấu hình bảo mật AWS bằng Terraform.

## Phạm vi chức năng

- Đăng ký, đăng nhập, đăng xuất và khôi phục mật khẩu bằng OTP.
- Quản lý tài liệu cá nhân: tải lên, tải xuống, yêu thích, thùng rác và khôi phục.
- Chia sẻ và thu hồi quyền xem tài liệu cho người dùng khác.
- Quản trị tài khoản, vai trò, quyền và xem nhật ký hoạt động.
- Lưu file ở local hoặc S3; lưu metadata ở SQLite hoặc MySQL.
- Cung cấp Docker, migration, các bước kiểm tra bảo mật trong CI và phương án triển khai AWS.
...
Các bước quản lý tài liệu đều cần kiểm tra danh tính và quyền của người dùng. Việc khôi phục tài khoản bằng OTP và theo dõi hoạt động cũng phải tuân theo cùng các quy tắc bảo vệ tài khoản, dữ liệu.
...
- Mô hình vai trò hiện có gồm `admin` và `user`; hệ thống dữ liệu có các quyền chi tiết.
...
- CSRF được bật toàn cục cho form và API thay đổi dữ liệu. Ứng dụng còn có rate limit, khóa tài khoản tạm thời, các header bảo mật và vô hiệu hóa session sau khi tài khoản hoặc mật khẩu thay đổi.
- Malware scanning, HTTPS với domain, AWS Backup nâng cao và kiểm thử khôi phục là các khả năng tùy chọn hoặc chưa bật vì phát sinh chi phí.
...
- RBAC và khu vực quản trị: `app/permissions.py`, `app/routes/admin.py`, `app/templates/admin/` và `docs/security/RBAC.md`.
...
- Bộ kiểm thử: `tests/`, `security/**/test_*.py`, kiểm tra tích hợp trong `scripts/` và các bước kiểm tra trong `.github/workflows/ci.yml`.
- Chưa có bằng chứng về chứng nhận bảo mật, pentest độc lập, benchmark hoặc một
  ...
  năng dự kiến để tuyên bố một biện pháp bảo mật cloud đang hoạt động thật.
...
5. Dễ chạy và mở rộng: giữ cách chạy trên máy cá nhân đơn giản với Flask và SQLite, đồng thời hỗ trợ Docker trên Ubuntu và triển khai AWS.
