...
Docker Compose chạy ứng dụng cùng MySQL và một bước cập nhật cơ sở dữ liệu. Cấu hình gồm các service sau:
...
`migrate` phải kết thúc với mã thoát 0 và `web`, `db` phải ở trạng thái
...
Nếu chỉ cần kiểm thử rồi dọn dẹp, dùng lệnh dưới đây. Script sẽ tạo một bộ container tạm với thông tin bí mật ngẫu nhiên và xóa đúng volume của lần chạy đó:
