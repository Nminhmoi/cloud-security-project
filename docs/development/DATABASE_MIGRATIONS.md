...
CloudBox định nghĩa dữ liệu bằng model SQLAlchemy và quản lý thay đổi cấu trúc bằng Flask-Migrate/Alembic. Khi chạy trên máy cá nhân, ứng dụng mặc định dùng SQLite. Cùng bộ model này được dùng cho MySQL trong Docker và RDS MySQL trên AWS.
...
Migration đầu tiên tạo sẵn hai vai trò `admin`, `user` cùng các quyền mặc định. Sau khi chạy migration, dùng `python admin.py` để tạo tài khoản quản trị.
