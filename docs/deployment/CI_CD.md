# CI/CD và kiểm tra bảo mật

CloudBox dùng GitHub Actions để kiểm tra mã nguồn và triển khai ứng dụng. Hai công việc này được tách thành các workflow riêng:

- `.github/workflows/ci.yml` chạy khi có pull request hoặc push lên `main`.
- `.github/workflows/deploy.yml` chỉ chạy khi người dùng chủ động khởi chạy từ GitHub Actions.
...
1. `Python tests` kiểm tra biên dịch mã nguồn và chạy unit test của ứng dụng cùng module
   security trên Python 3.12, trùng với Docker image.
2. `Python security` chạy `pip-audit` cho thư viện cần khi chạy ứng dụng và Bandit cho mã
   ứng dụng. Cảnh báo Bandit mức High sẽ làm job thất bại.
...
merge. CI chỉ có quyền `contents: read`, không có quyền cấp token AWS hoặc
deployment secret.

Để chạy các bước kiểm tra trên máy cá nhân, dùng các lệnh sau:
...
| Variable | `AWS_ROLE_ARN` | ARN của OIDC deployment vai trò |
| Variable | `TF_STATE_BUCKET` | Terraform state bucket riêng tư đã tồn tại |
...
audience `sts.amazonaws.com`, sau đó cấu hình trust policy của deployment vai trò
...
lý. Tránh `AdministratorAccess`; deployment vai trò phải tách biệt với EC2
application vai trò có phạm vi nhỏ hơn nhiều.
